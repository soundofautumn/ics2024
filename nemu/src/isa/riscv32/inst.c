...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t = cpu.csr[imm]; cpu.csr[imm] = src1; R(rd) = t; printf("csr[%d] = " FMT_WORD "\n", imm, cpu.csr[imm]); );
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, cpu.csr[imm] |= src1; R(rd) = cpu.csr[imm]; printf("csr[%d] = " FMT_WORD "\n", imm, cpu.csr[imm]); );
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, cpu.csr[imm] &= ~src1; R(rd) = cpu.csr[imm]; printf("csr[%d] = " FMT_WORD "\n", imm, cpu.csr[imm]); );
