...
enum {
  TYPE_R, TYPE_I, TYPE_I_shamt, TYPE_S, TYPE_B, TYPE_U, TYPE_J,
  TYPE_N, // none
};
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I_shamt, R(rd) = (int32_t)src1 >> imm);
  ...
  // system
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = isa_raise_intr(ECALL_M, s->pc); 
                                                                IFDEF(CONFIG_ETRACE, Log("ecall with NO = %d, pc = 0x%x", GPR1, s->pc)); );
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0

  // RV32/RV64 Zicsr Standard Extension
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t = cpu.csr[imm]; cpu.csr[imm] = src1; R(rd) = t;);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, cpu.csr[imm] |= src1; R(rd) = cpu.csr[imm];);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, cpu.csr[imm] &= ~src1; R(rd) = cpu.csr[imm];);
